...
struct FavoriteTeamWidgetView: View {
    ...
    var body: some View {
            ...
            VStack {
                ...
                .padding(
                    widgetFamily == .systemLarge
                        || widgetFamily == .systemExtraLarge ? 5 : 1)
            }
        ...
        case .accessoryCircular:
            let value =
                entry.TableRow.wins_count / Double(entry.TableRow.match_count)
            ...
            ZStack {
                ...
                VStack {
                    ...
                    Text(
                        "**\(Int(entry.TableRow.wins_count))-\(Int(entry.TableRow.losses_count))**"
                    )
                }
            }
            .containerBackground(for: .widget) {}
        ...
        case .accessoryRectangular:
            ZStack {
                HStack {
                    ...
                    if !entry.team.league_entries.isEmpty {
                        Text("\(entry.team.name)")
                            + Text(
                                " (\(entry.team.league_entries[0].league.acronym))"
                            )
                ...
                Text(
                    "Record: **\(Int(entry.TableRow.wins_count))-\(Int(entry.TableRow.losses_count))**"
                )
                Text("Rank: **\(entry.TableRow.rank)**")
        ...
        case .accessoryInline:
            HStack {
                ...
                if !entry.team.league_entries.isEmpty {
                    Text(
                        "\(entry.team.name): \(Int(entry.TableRow.wins_count))-\(Int(entry.TableRow.losses_count))"
                    )
                        + Text(
                            " (\(entry.team.league_entries[0].league.acronym))"
                        ).foregroundColor(.secondary)
                ...
                VStack(alignment: .leading, spacing: 0.0) {
                    HStack {
                        ...
                        if let awayScore = lastGame.away_runs,
                            let homeScore = lastGame.home_runs
                        {
                ...
                VStack(alignment: .leading, spacing: 10.0) {
                    ...
                    HStack {
                        ...
                        if let gameDate = nextGame.gameDate {
                            Text(
                                gameDate,
                                format: Date.FormatStyle().day().month().year()
                            )
                            .scaledToFill()
                        }
                    }
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack(alignment: .bottom) {
                    ...
                    Group {
                        HStack {
                            Image(systemName: "sum")
                                .frame(maxWidth: 20)
                            Text(
                                String(Int(entry.TableRow.wins_count)) + "-"
                                    + String(Int(entry.TableRow.losses_count))
                            )
                            .bold()
                        }
                        HStack {
                ...
                .background(
                    ContainerRelativeShape().fill(Color.secondaryBackground)
                )
                .font(.subheadline)
            }
...
struct FavoriteTeamWidgetView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        let dummyEntry = FavoriteTeamEntry(
            ...
            lastGameRoadLogo: TeamImageData.flamingosLogo,
