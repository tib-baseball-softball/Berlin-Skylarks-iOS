    ...
    var body: some View {
        ScrollView {
            ...
            LazyVGrid(columns: columns) {
                ...
                {
                    ClubGridItem(
                        systemImage: "info.circle.fill", itemName: "Details"
                    )
                    .padding(3)
                }
                NavigationLink(
                    destination: UmpireView(licenseManager: licenseManager)
                ) {
                    ClubGridItem(
                ...
                NavigationLink(
                    destination: ScorersView(licenseManager: licenseManager)
                ) {
                ...
                NavigationLink(destination: TeamListView()) {
                    ClubGridItem(
                        systemImage: "person.3.fill", itemName: "Teams"
                    )
                    .padding(3)
                }
                NavigationLink(destination: BallparkView(clubData: clubData)) {
                    ClubGridItem(
                        systemImage: "diamond.fill", itemName: "Ballpark"
                    )
                    .padding(3)
                }
                ...
                {
                    ClubGridItem(
                        systemImage: "person.2.fill", itemName: "Officials"
                    )
                    .padding(3)
                }
            }
