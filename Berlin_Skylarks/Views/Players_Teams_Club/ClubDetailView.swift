            ...
            Section(header: Text("Team Basics")) {
                ...
                HStack {
                    Image(systemName: "creditcard")
                        .clubIconStyleRed()
                    Text(
                        "\(clubData.club.acronym) / 0\(clubData.club.organization_id) \(clubData.club.number)"
                    )
                }
                HStack {
            ...
            Section(
                ...
                HStack {
                    Image(systemName: "map")
                        .clubIconStyleDynamic()
                    Text(
                        "\(clubData.club.street), \(clubData.club.postal_code) \(clubData.club.city), \(clubData.club.country)"
                    )
                    .textSelection(.enabled)
                }
                MapViewWithPin(
                    latitude: clubData.club.latitude,
                    longitude: clubData.club.longitude, name: clubData.club.name
                )
            }
            .listRowSeparatorTint(.skylarksSand)
        }
