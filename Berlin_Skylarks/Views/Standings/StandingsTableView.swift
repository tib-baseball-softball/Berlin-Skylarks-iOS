...
///Represents a single table with ONE league. it can be accessed by tapping the corresponding league in StandingsView.
                    ...
                    HStack {
                        ...
                        HStack {
                            Text(String(Int(tableRow.wins_count)))
                                .frame(
                                    width: 21, height: 20, alignment: .center)
                            Text(String(Int(tableRow.losses_count)))
                                .frame(
                                    width: 21, height: 20, alignment: .center)
                            Text(tableRow.quota)
                                .frame(
                                    width: 42, height: 20, alignment: .center)
                            Text(String(tableRow.games_behind))
                                .fixedSize()
                                .frame(
                                    width: 31, height: 20, alignment: .center)
                            Text(tableRow.streak)
                                .frame(
                                    width: 37, height: 20, alignment: .center)

                        }
                        .padding(.horizontal, -8)
                    }
                    .foregroundColor(
                        tableRow.team_name.contains("Skylarks")
                            ? Color.accentColor : Color.primary)
                }
            }
            .frame(maxWidth: 650)
            .navigationTitle(
                leagueTable.league_name + " " + String(leagueTable.season)
            )
            #if !os(macOS)
...
struct StandingsTableView_Previews: PreviewProvider {
    static var previews: some View {
        StandingsTableView(leagueTable: emptyTable)
