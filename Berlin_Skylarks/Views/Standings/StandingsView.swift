...
/// the overview table where the desired league is selected to display standings
import SwiftUI
...
struct StandingsView: View {
    ...
    func loadAllTables() async {
        ...
        for leagueGroup in leagueGroups {
            let url = URL(
                string: "https://bsm.baseball-softball.de/leagues/"
                    + "\(leagueGroup.id)" + "/table.json")!

            do {
                let table = try await fetchBSMData(
                    url: url, dataType: LeagueTable.self)
                leagueTables.append(table)
            } catch {
                print(
                    "Request failed with error: \(error) while parsing \(leagueGroup.name) with id \(leagueGroup.id)"
                )
            }
        }
                            ...
                            HStack {
                                ...
                                HStack {
                                    ...
                                    if !favTeam.league_entries.isEmpty {
                                        ...
                                        if favTeam.league_entries[0].league.name
                                            == leagueTable.league_name
                                        {
